...
http.createServer(function(req,res){
    ...
    if(pathname=='/favicon.ico'){
        ...
        fs.exists(filename,function(exists){ //判断文件是否存在
            if(exists){ //文件存在
                var htmlStr = '<link href="/public/css/index.css" rel="stylesheet" />';
                htmlStr += "<h1>静态目录、文件删除、查看</h1><ul>";
                                ...
                                if(file=='.git'||file=='node_modules'||file=='.idea'){
                                    ...
                                    if(pathname=='/'){//根目录
                                        if(fs.statSync(filePath).isDirectory()){
                                            htmlStr+='<li class="folder"><a href="'+file+'">'+file+'</a><a href=/del?path='+file+' class="del">删除</a> </li>';
                                        }else{
                                            htmlStr+='<li><a href="'+file+'">'+file+'</a><a href=/del?path='+file+' class="del">删除</a> </li>';
                                        }
                                    }else{ //根目录下一级目录
                                        if(fs.statSync(filePath).isDirectory()){
                                            htmlStr+='<li class="folder"><a href="'+pathname+'/'+file+'">'+file+'</a><a href=/del?path='+pathname+'/'+file+' class="del">删除</a> </li>';
                                        }else{
                                            htmlStr+='<li><a href="'+pathname+'/'+file+'">'+file+'</a><a href=/del?path='+pathname+'/'+file+' class="del">删除</a> </li>';
                                        }
                                    }
                                }
                            })
